           ...
           $("#modalfooter").on("click", ".save-btn-cf", function (e) {
            ...
            async function saveData(){
                ...
                function getOrderSheet1(){                    
                    ...
                    for(let i = 0; i < select_batt_data.length; i++){
                        if(select_batt_data[i].main == true){
                            if(select_batt_data[i].wh_stock < select_batt_data[i].amount || select_batt_data[i].wh_stock != select_batt_data[i].amount){
                            var new_order = select_batt_data[i].amount - select_batt_data[i].wh_stock
                            order_battery.push({
                                no:select_batt_data[i].no,
                                main:select_batt_data[i].main,
                                lock:select_batt_data[i].lock,
                                series:select_batt_data[i].series,
                                company:select_batt_data[i].company,
                                origin:select_batt_data[i].origin,
                                warranty:select_batt_data[i].warranty,
                                amount:select_batt_data[i].amount,
                                inter_confirm:select_batt_data[i].inter_confirm,
                                wh_stock:select_batt_data[i].wh_stock,
                                deliverydate:select_batt_data[i].deliverydate,
                                order_amount:new_order
                            })
                                
                            }
                            all_origin.push(select_batt_data[i].origin)
                            all_warranty.push(select_batt_data[i].warranty)
                            all_delivery.push(select_batt_data[i].deliverydate)
                        }
                        else{
                            if(select_batt_data[i].wh_stock < select_batt_data[i].amount || select_batt_data[i].wh_stock != select_batt_data[i].amount){
                            var new_order = select_batt_data[i].amount - select_batt_data[i].wh_stock
                            new_order_battery.push({
                                no:select_batt_data[i].no,
                                main:select_batt_data[i].main,
                                lock:select_batt_data[i].lock,
                                series:select_batt_data[i].series,
                            ...
                            new_all_origin.push(select_batt_data[i].origin)
                            new_all_warranty.push(select_batt_data[i].warranty)
                            new_all_delivery.push(select_batt_data[i].deliverydate)
                        }
                    ...
                    for(let i = 0 ; i < uniq.length;i++){
                        for(let j = 0; j < ordercondition.length;j++){
                            if(uniq[i] == ordercondition[j].condition){
                                uni_condition.push({order:ordercondition[j].order,condition:ordercondition[j].condition,no:(i+1).toString()})
                                lastcondition.push((i+1).toString())
                            }
                        }
                    ...
                    for(let i = 0 ; i < new_uniq.length;i++){
                        for(let j = 0; j < new_ordercondition.length;j++){
                            if(new_uniq[i] == new_ordercondition[j].condition){
                                new_uni_condition.push({order:new_ordercondition[j].order,condition:new_ordercondition[j].condition,no:((i+1)).toString()+'(R)'})
                                lastcondition.push(((i+1)).toString()+'(R)')
                ...
                function getOrderSheet8(){
                    console.log("OrderSheet : ",OrderSheet)
                    console.log("new_OrderSheet : ",new_OrderSheet)
                    console.log("uni_condition : ",uni_condition)
                    console.log("new_uni_condition : ",new_uni_condition)
                        ...
                        for(let i = 0;i < uni_condition.length;i++){
                            if(uni_condition[i].no == OrderSheet[j].no){
                                ...
                                OrderSheet[j].battery.push({
                                    ...
                                    lock:order_battery[uni_condition[i].order].lock,
                                    ...
                                    amount:order_battery[uni_condition[i].order].amount,
                                    inter_confirm:order_battery[uni_condition[i].order].inter_confirm,
                                    wh_stock:order_battery[uni_condition[i].order].wh_stock,
                        ...
                        for(let i = 0;i < new_uni_condition.length;i++){
                            ...
                            OrderSheet[j].origin = new_order_battery[new_uni_condition[i].order].origin
                             OrderSheet[j].battery.push({
                                    ...
                                    lock:new_order_battery[new_uni_condition[i].order].lock,
                                    ...
                                    amount:new_order_battery[new_uni_condition[i].order].amount,
                                    inter_confirm:new_order_battery[new_uni_condition[i].order].inter_confirm,
                                    wh_stock:new_order_battery[new_uni_condition[i].order].wh_stock,
