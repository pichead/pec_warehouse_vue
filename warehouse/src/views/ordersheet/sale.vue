    ...
    mounted() {
                        ...
                        if(Project.data().visible == true){
                            ...
                            $('#data').append(
                                '<tr>'+
                                    ...
                                    '<td>'+
                                        '<button class="btn btn-info p-0 px-1 col-5 ml-1 del-btn" value="'+Project.id+'" data-toggle="modal" data-target="#viewModal" data-jobid="'+Project.id+'" >view</button>'+
                                    '</td>'+
                                '</tr>'
                            )
                            i++
        ...
        $("#data").on("click", ".del-btn", function (e) {
            ...
            projectFirestore.collection("Projects").doc(jobnoid).get().then(jobdata =>{
                ...
                console.log('order_orderSheet_list : ',order_orderSheet_list)
                for(let i = 0; i < order_orderSheet_list.length; i++){
                    var sale_cf
                    var wh_cf
                    if(order_orderSheet_list[i].sale_confirm == true){
                        sale_cf  = '<select id="sale-order-option'+(i+1)+'" class="form-control form-control-sm confirm-option" required>'+
                    ...
                    if(order_orderSheet_list[i].warehouse_confirm == true){
                        wh_cf  = '<td id="wh_cf'+(i+1)+'" class="text-success">ยืนยัน</td>'
                    }
                    else{
                        wh_cf  = '<td id="wh_cf'+(i+1)+'" class="text-danger">ไม่ยืนยัน</td>'
                    }
                ...
                for(let i = 0; i < order_booking_list.length; i++){
                    var booking_sale_cf
                    var booking_wh_cf
                    if(order_booking_list[i].sale_confirm == true){
                        booking_sale_cf  = '<select id="sale-booking-option'+(i+1)+'" class="form-control form-control-sm confirm-option" required>'+
                                    '<option value="false" >ไม่ยืนยัน</option>'+
                                    '<option selected  value="true">ยืนยัน</option>'+
                                '</select>'
                    }
                    ...
                    if(order_booking_list[i].warehouse_confirm == true){
                        booking_wh_cf  = '<td id="booking_wh_cf'+(i+1)+'" class="text-success">ยืนยัน</td>'
                    }
                    else{
                        booking_wh_cf  = '<td id="booking_wh_cf'+(i+1)+'" class="text-danger">ไม่ยืนยัน</td>'
                    }
            ...
            async function saveData(){
                ...
                function GetAllData(){
                    for(let i = 0; i < orderSheetNo; i++){
                        ...
                        var sale_cf
                        var wh_cf
                        console.log($('#sale-order-option'+(i+1)).find('option:selected').val())
                        console.log($('#wh-order-option'+(i+1)).find('option:selected').val())

                        if($('#sale-order-option'+(i+1)).find('option:selected').val() == 'true'){
                            sale_cf = true
                        }
                        else{
                            sale_cf = false
                        }
                        if($('#wh_cf'+(i+1)).text() == 'ยืนยัน'){
                            wh_cf = true
                        }
                        else{
                            wh_cf = false
                    ...
                    for(let i = 0; i < bookingNo; i++){
                        ...
                        var booking_sale_cf
                        var booking_wh_cf
                        console.log($('#sale-booking-option'+(i+1)).find('option:selected').val())
                        console.log($('#wh-booking-option'+(i+1)).find('option:selected').val())

                        if($('#sale-booking-option'+(i+1)).find('option:selected').val() == 'true'){
                            booking_sale_cf = true
                        }
                        else{
                            booking_sale_cf = false
                        }
                        if($('#booking_wh_cf'+(i+1)).text() == 'ยืนยัน'){
                            booking_wh_cf = true
                        }
                        else{
                           booking_wh_cf = false
